...
Offline only. Numeric parameters from 03-00 receive inferred write definitions;
unknown special formats remain read-only. No request addresses are invented.
The session's 06 23 writes are not mixed into the read catalog.
"""
...
def value_model(ident, data):
    ...
    if ident == "03-07" and (typ, unit) == (0x0D, 6):
        return "Zahlenantwort<Faktor10>", "SIN / 10 ohne Einheit; aus Antwortgrenze 100 und STE-Grenze 10 abgeleitet"
...
def build_controller_catalog(records, ste):
    ...
    for item in scanned:
        ...
        if item["tem_id"] == "07-09":
            item["label"], item["label_source"] = "Solltemperatur Handbetrieb", "Benutzerbestaetigung"
        result.append(item)
    ...
    parameters = canonical_parameters(result)
    # Extend writes AFTER choosing preferred selectors, preserving alias ranking
    # and the historical had_write metadata used by the migration report.
    for item in parameters:
        if tuple(map(int, item["tem_id"].split("-"))) >= (3, 0):
            item["generate_write"] = write_model(item["response_model"]) is not None
            if not item["generate_write"]:
                item["write_exclusion"] = "Kurz-/Sonderantwort: Schreibformat nicht bekannt"
    return directory + sorted(parameters, key=lambda e: (tuple(map(int, e["tem_id"].split("-"))), e["request"]))
...
// Numeric parameters from 03-00 also receive inferred writes on scan-only selectors.
// Short/special responses without a known value encoding remain read-only; device acceptance is unverified.
...
/** 03-07: Antwortgrenze 100 / STE-Grenze 10; ohne Einheit. */
@divisor(10) scalar Faktor10 extends SIN;
...
def render_tsp(catalog):
    ...
    for entry in catalog:
        ...
        if entry["service"] == "0621" and value_type and entry.get("generate_write", True):
            result += [f"  /** {comment} Schreibwert; derselbe Selektor wird als 06 23 an 10 verwendet. Annahme des Schreibbefehls durch den Regler nicht fuer jeden Parameter bestaetigt. */",
                       "  @write",
                       "  @zz(0x10)",
...
def write_report(source, records, errors, catalog, ste, output):
    ...
    lines = ["# Bevorzugte Zugriffe aus commands.txt und input/complete.log", "",
             ...
             "Numerische Parameter ab 03-00 erhalten auch fuer reine Scan-Zugriffe Schreibmodelle; die Annahme durch den Regler ist damit nicht bestaetigt.", "",
    ...
    lines += ["", "## Schreibdefinitionen ab 03-00", "",
              "Alle numerisch dekodierbaren Parameter ab 03-00 besitzen ein `_set`-Modell "
              "mit gleichem Selektor, Dienst `06 23` und Ziel `10`. Vorhandene Schreibmodelle "
              "unterhalb 03-00 bleiben erhalten. Die Definitionen belegen keine Annahme durch "
              "den Regler; insbesondere reine Scan-Zugriffe wurden nicht live schreibend geprüft.", "",
              "Folgende Zugriffe bleiben wegen unbekannter Schreibstruktur ausgenommen:", "",
              "| Modell | TEM | Selektor | Grund |", "|---|---|---|---|"]
    for item in catalog:
        if item.get("write_exclusion"):
            lines.append(f"| `{item['name']}` | {item['tem_id']} | `{item['request']}` | {item['write_exclusion']} |")
