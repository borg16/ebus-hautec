...
request addresses for parameters absent from the capture.
"""
    ...
    for record in records:
        if record["status"] == "complete":
            if (record["source"], record["destination"]) != ("01", "15"):
                raise ValueError("unexpected source/destination; review new capture before generating")
...
TSP_HEADER = '''// Generated by scripts/analyze_master_log.py from CRC-checked master.log.
...
def main():
    ...
    parser.add_argument("source", type=Path, nargs="?", default=ROOT / "master.log")
